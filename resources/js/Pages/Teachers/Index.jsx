import AdminLayout from '@/Layouts/AdminLayout';
import Table from '@/Components/Admin/Table';
import DeleteButton from '@/Components/Admin/DeleteButton';
import Pagination from '@/Components/Admin/Pagination';
import { Link, router, usePage } from '@inertiajs/react';

export default function Index() {
    const { teachers } = usePage().props;
    const data = teachers.data || teachers;
    const pagination = teachers.links ? teachers : null;

    const handleDelete = (id) => {
        router.delete(route('teachers.destroy', id));
    };

    const headers = ['الكود', 'الاسم', 'البريد', 'التخصص', 'عدد المجموعات'];
    const rows = (data || []).map((t) => ({
        cells: [
            t.teacher_code || '-',
            t.user?.name || '-',
            t.user?.email || '-',
            t.specialization || '-',
            t.groups_count ?? 0,
        ],
    }));

    return (
        <AdminLayout title="المدرسين">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white">المدرسين</h1>
                    <p className="text-gray-500 dark:text-gray-400">إدارة بيانات المدرسين</p>
                </div>
                <Link
                    href={route('teachers.create')}
                    className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700"
                >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                    </svg>
                    إضافة مدرس
                </Link>
            </div>

            <div className="overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
                <Table
                    headers={headers}
                    rows={rows}
                    actions={(row, index) => (
                        <div className="flex items-center gap-3">
                            <Link
                                href={route('teachers.edit', data[index].id)}
                                className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300"
                                title="تعديل"
                            >
                                <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </Link>
                            <DeleteButton onDelete={() => handleDelete(data[index].id)} itemName={data[index].user?.name || data[index].teacher_code} />
                        </div>
                    )}
                />
                {pagination && <div className="border-t border-gray-200 px-4 py-3 dark:border-gray-700"><Pagination links={pagination.links} /></div>}
            </div>
        </AdminLayout>
    );
}
